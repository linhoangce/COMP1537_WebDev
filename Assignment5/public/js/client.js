...
function ajaxGET(url, callback) {
    ...
    xhr.open("GET", url); // localhost:8000/...
    xhr.send();
}

/* 
 * Assignment5 Code Section
 */

let homeContent = document.getElementById('card').innerHTML; // This is where the content of ajaxGET function is displayed

document.getElementById('artist').addEventListener('click', function(e) {
    console.log("Clicked");
    ajaxGET('/table-async?format=html', function(data) {
        ...
        header.innerHTML = "<h2>Assignment 5 - DATABASE</h2>";
...
document.getElementById("aution").addEventListener("click", function(e) {
    console.log("Clicked");
    ajaxGET('/table-join?format=html', function(data) {
        let container = document.createElement('div');
        let div = document.createElement('div');
        let header = document.createElement('div');
        div.setAttribute('id', 'user-post');
        header.setAttribute('id', 'post-table-header');
        header.innerHTML = "<h2>Posts Data</h2>";
        div.innerHTML = data;
        container.appendChild(header);
        container.appendChild(div);
